def call(){
    ...
    parameters {
        choice(name: 'ENV_SPEC', choices: ['test63','cert1','prod3,prod4,prod5,prod6'],description: 'Выбирите куда хотите запустить Deploy')
    ...
    environment { // блок для описания переменных окружения
        ...
        REPO_NAME="laravel"
        APP_REPO_URL="https://gitlab.rosbank.rus.socgen/proport/laravel.git" // адрес репозитория
        ...
        NEXUS_REPOSITORY_URL="https://nexus.gts.rus.socgen/repository/proportal-raw/laravel/"
        ...
        steps {
            script {
                ...
                if (branchName.isEmpty()) {
                    ...
                    print "DEBUG: parameter ENV_SPEC = ${params.ENV_SPEC}"
                    print "DEBUG: parameter BUILD = ${BUILD_NUMBER}"
                    print "DEBUG: parameter BUILD = ${branchName}"
                ...
                success {
                    script {
                        ...
                        def jsonData = [
                            "token":"${telegram_api}",
                            "id":"-1001845199484",
                            "text":"Завершена сборка Laravel в CERT, билд ветки ${params.BRANCH_NAME}, результат: ${currentBuild.result}. Запустил: ${lastname}"
                        ]
...
def buildAndPush() {
    datetime=sh(script: "date +%Y%m%d%H%M", returnStdout: true)
    artifact_name="laravel---" + datetime.trim()+".zip"
    ...
    withCredentials([usernamePassword(credentialsId: "${CREDENTIALSID}", passwordVariable: 'PASSWORD', usernameVariable: 'USERNAME')]){
        sh 'curl -k -u $USERNAME:$PASSWORD --upload-file ${artifact_name} ${NEXUS_REPOSITORY_URL}'
    }
}
